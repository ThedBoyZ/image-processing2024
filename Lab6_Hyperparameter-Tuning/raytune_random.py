...
def save_model(model, dir, epoch, last=False):
    ...
    file_name = "model-at-" + cur_time.strftime('%d-%m-%y-%H%M%S') + "epoch-" + str(epoch)
    save_path = dir + file_name
    try:
        try:
            if not os.path.exists(dir):
                os.makedirs(dir)
            if last: torch.save(model, dir + "last_model")
            else: torch.save(model, save_path)
        except:
            torch.save(model, "backup_from_path=" + file_name)
    except:
...
def test_train():
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    report = dict({})
    train(model, optimizer, loss_fn, trainloader, testloader, epochs=3, save_dir="/home/tkrittithee/blue-workspace/Lab6_Hyperparameter-Tuning/models", device=device, report=report)
    print(report)
...
def do_ray_rando():
    ...
    config = {
        ...
        "batch_size": tune.randint(16, 32 + 1), # make it inclusive
        "optimizer": tune.choice(["Adam", "SGD"]),
        "loss_fn": nn.MSELoss(),
        "epochs": tune.randint(10, 100 + 1), # make it inclusive
        "lr": tune.uniform(1e-4, 1e-2),
    }
    tuner = tune.Tuner(
        tune.with_resources(
            train_raytune,
            resources={"gpu": 0.5, "cpu":30}
        ),
        tune_config = tune.TuneConfig(
            metric = 'val_psnr',
            mode = 'max',
